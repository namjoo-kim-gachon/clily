...
type CreateTerminalRuntimeOptions = {
  ...
  sessionName?: string
  force?: boolean
  // true for brand-new sessions: auto-injects OSC 7 shell integration
  isNewSession?: boolean
}

const HOME = process.env.HOME ?? process.cwd()
const CLILY_SHARE_DIR = join(HOME, ".local", "share", "clily")
const SHELL_INTEGRATION_PATH = join(CLILY_SHARE_DIR, "shell-integration.sh")

// Shell integration script: hooks into zsh/bash precmd to emit OSC 7 cwd on every prompt.
// Using precmd (not chpwd) so cwd is also emitted on shell startup/reattach.
const SHELL_INTEGRATION_SCRIPT = `#!/bin/sh
# Auto-generated by clily — do not edit manually
if [ -n "$ZSH_VERSION" ]; then
  autoload -Uz add-zsh-hook
  _clily_cwd() { printf '\\033]7;file://%s%s\\007' "\${HOST:-$(hostname)}" "\$PWD"; }
  add-zsh-hook precmd _clily_cwd
elif [ -n "$BASH_VERSION" ]; then
  _clily_cwd() { printf '\\033]7;file://%s%s\\007' "\${HOSTNAME:-$(hostname)}" "\$PWD"; }
  PROMPT_COMMAND="\${PROMPT_COMMAND:+\$PROMPT_COMMAND; }_clily_cwd"
fi
`

function ensureShellIntegration(): void {
  try {
    mkdirSync(CLILY_SHARE_DIR, { recursive: true })
    writeFileSync(SHELL_INTEGRATION_PATH, SHELL_INTEGRATION_SCRIPT, { mode: 0o755 })
  } catch {
    // Non-fatal — OSC 7 won't be auto-configured for new sessions
  }
}
...
export function createTerminalRuntime(
  ...
  // For new sessions: inject shell integration after the first stable pause in output.
  // The timer resets on each data chunk; fires 400 ms after the last chunk.
  // If OSC 7 already arrives (shell already configured), the injection is cancelled.
  let integrationTimer: ReturnType<typeof setTimeout> | null = null
  let integrationPending = options?.isNewSession === true

  const cancelIntegration = () => {
    integrationPending = false
    if (integrationTimer) { clearTimeout(integrationTimer); integrationTimer = null }
  }
  ...
  const scheduleIntegration = () => {
    ...
    integrationTimer = setTimeout(() => {
      if (integrationPending && !disposed) {
        integrationPending = false
        terminal.write(`source ${SHELL_INTEGRATION_PATH}; clear\r`)
      }
    }, 400)
  }
  ...
  const unsubscribeData = terminal.onData((data) => {
    ...
    if (cwdMatch?.[1]) {
      // OSC 7 received — shell already has integration, no need to inject
      cancelIntegration()
      try {
    ...
    if (integrationPending) scheduleIntegration()
    broadcast(data)
  })
